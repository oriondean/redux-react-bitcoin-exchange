...
export default function reduce(state = initialState, action) {
  // convert key to string to allow usage in immutable map
  const id = action.data && action.data.id ? action.data.id.toString() : '';

  switch (action.type) {
    case ORDER_BOOK_UPDATES.REFRESH: {
      // Transform array of orders into map (id -> order)
      const ordersById = action.data.reduce((map, order) => ({ ...map, [order.id]: order }), {});
      return Immutable.fromJS(ordersById);
    }
    case ORDER_BOOK_UPDATES.ADD_ORDER:
      return state.set(id, Immutable.fromJS(action.data));
